...
local function sendMessage(clientId, type, data)
    log("Sending message with type '" ..
        type .. "' to " .. currentProtocol .. "://" .. clientId .. " with data: " .. textutils.serialize(data))
...
local wrappedPeripheralApi = {
    ...
    call = function(clientId, peripheralName, methodName, ...)
        local args = ...
        log("Real call(" .. clientId ..
            ", " .. peripheralName .. ", " .. methodName .. ", " .. textutils.serialize(args) .. ")")
    ...
    if event[1] == "rednet_message" then
        if event[4] == currentProtocol then
            if event[3].version and event[3].version == CURRENT_VERSION then
                ...
                log("Recieved event from an unsupported WPP version. Only version '" ..
                    CURRENT_VERSION .. "' is supported.")
            end
        end
    end
    if event[1] == "monitor_touch" then
        monTouchTableText = textutils.serialise(event)
        print(monTouchTableText)
        rednet.broadcast(monTouchTableText)
    end
end
...
function wireless.prefetchMethods(peripheralUrl, methods)
    ...
    if parsedPeripheralUrl == nil then
        ...
        sendMessage(parsedPeripheralUrl.clientId, "function",
            { func = "wppPrefetch", args = { parsedPeripheralUrl.peripheralId, methods } })
...
function remotePeripheral.isPresent(peripheralUrl)
    ...
    if parsedPeripheralUrl == nil then
        ...
        sendMessage(parsedPeripheralUrl.clientId, "function", { func = "isPresent",
            args = { parsedPeripheralUrl.peripheralId } })
        local reply = recieveReply()
...
function remotePeripheral.getMethods(peripheralUrl)
    ...
    if parsedPeripheralUrl == nil then
        ...
        sendMessage(parsedPeripheralUrl.clientId, "function",
            { func = "getMethods", args = { parsedPeripheralUrl.peripheralId } })
        local reply = recieveReply()
...
function remotePeripheral.call(peripheralUrl, method, ...)
    ...
    if parsedPeripheralUrl == nil then
        ...
        sendMessage(parsedPeripheralUrl.clientId, "function",
            { func = "call", args = { parsedPeripheralUrl.peripheralId, method, { ... } } })
        local reply = recieveReply()
