...
--    - Near final update for converting to BigReactors
--
--  Version 3.5 - September 27/21
--    - Changed "BigReactors" text to "BigReactors"
--
...
-----------------------------------------------
-- Reactor Control Version History
-- Version 3.8 11/22 Updated to work with extreme reactors (minecraft 1.18)
-----------------------------------------------
...
--name your reactor--
local ReactorName = "Passive Reactor #1"

-- WPP settings --
local usewpp = false
local wpp_peripheral_reactor = "wpp@reactor://17/BigReactors-Reactor_1"
local peripheral_monitor = "monitor_12"
...
function menu_bar()
  ...
  draw_text(2, 19, "     " .. ReactorName, colors.white, colors.blue)
end
...
function homepage()
  while true do
    ...
    if reactor.getEnergyCapacity() ~= null then
    ...
    if reactor.isActivelyCooled() == true then
      ...
      energy_stored_percent = math.floor((energy_stored / reactor.getEnergyCapacity()) * 100)
      draw_text(13, 15, energy_stored_percent .. "% (" .. math.floor(energy_stored) .. " FE)", colors.white, colors.black)
    end
...
function install_update(program, pastebin)
  ...
  shell.run("pastebin get mcgdnR3K install")
  shell.run("install")
end
...
function update()
  ...
  shell.run("pastebin get TkGbX7pM current_version.txt")
...
function terminal_screen()
  ...
  draw_text_term(13, 19, "modified by brownbie", colors.white, colors.blue)
  ...
  draw_text_term(1, 7, "pastebin.com/mcgdnR3K", colors.blue, colors.black)
...
function reactorSearch()
  if usewpp == true then
    if fs.exists("wpp") then
      return wpp.peripheral.wrap(wpp_peripheral_reactor)
    end
  else
